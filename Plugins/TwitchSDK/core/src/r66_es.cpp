...
#define ES_WEBSOCKET_DEV_URL STR("ws://127.0.0.1:8080/ws"sv)
...
#define ES_HELIX_DEV_URL STR("http://127.0.0.1:8080/eventsub/subscriptions"s)
...
ESSubscription UserMessageSent(STR("channel.chat.message"s), STR("1"s), "broadcaster_user_id"s);
...
ES_IMPL_EVENT(UserMessageSent, ChannelChatMessage)
...
const std::map<EventStreamKind, ESMetadata> MetaMap = {
	...
	ES_MM_ENTRY(ChannelChatMessage, UserMessageSent, &UserMessageSent),
};
